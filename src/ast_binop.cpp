...
void BinaryOperation::mipsPrint(){
  if(dynamic_cast<Variable*>(lExp)){
    ...
    auto RHS = global_context->register_chunk(makeUNQ("__BinOP_RHS"), integer_type);
...
void BinaryOperation::mipsPrint(ChunkPtr res){
    TypePtr integer_type = std::make_shared<PrimitiveType>();
    auto LHS = global_context->register_chunk(makeUNQ("__BinOP_LHS"), integer_type);
    auto RHS = global_context->register_chunk(makeUNQ("__BinOP_RHS"), integer_type);
    if(lExp) lExp->mipsPrint(LHS);
    if(rExp) rExp->mipsPrint(RHS);
    int regL = LHS->load();
    int regR = RHS->load();
    int regRes = res->load();
