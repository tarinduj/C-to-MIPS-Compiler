...
void List::pyPrint(std::ostream& os){
  for(int i = 0; i < elements.size(); i++){
    if(elements[i]){
      //os<<type<<": ";
      if(i != 0 && type == "init") printIndent(os);
      if(type!="init") printIndent(os);
...
void List::getDeclaredVarNames(std::vector<std::string>& v) const{
  if(type == "init"){
    for(int i = 0; i < elements.size(); i++){
      if(elements[i]) elements[i]->getDeclaredVarNames(v);
    }
  }
}
...
void Scope::pyPrint(std::ostream& os){
  addIndent();
  //os<<"decList: ";
  if(decList) decList->pyPrint(os);
  os<<"\n";
  //os<<"statList: ";
  if(statList) statList->pyPrint(os);
  delIndent();
}
